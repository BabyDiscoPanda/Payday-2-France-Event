					...
					if player_importance_wgt then
						...
						if ubase then
							...
							if ubase.is_local_player then
								e_fwd = attention_info1.fs_ext_movement:m_head_rot():y()
							elseif ubase.is_husk_player then
								e_fwd = attention_info1.fs_ext_movement:detect_look_dir()
							end
							if e_fwd then
	...
	for i = detected_obj_i[0], 1, -1 do
		...
		if player_importance_wgt and attention_info2.is_human_player then
			...
			if attention_info2.is_husk_player then
				e_fwd = attention_info2.fs_ext_movement:detect_look_dir()
			else
				e_fwd = attention_info2.fs_ext_movement:m_head_rot():y()
			end
			local dot = mvec3_dot(e_fwd, tmp_vec1)
			weight = weight * weight * (1 - dot)
			player_importance_wgt_nr = player_importance_wgt_nr + 1
...
function CopLogicBase._get_logic_state_from_reaction(data, reaction)
	...
	local police_call = _is_loud or managers.groupai._state:chk_enemy_calling_in_area(managers.groupai._state:get_area_from_nav_seg_id(data.fs_ext_movement:nav_tracker():nav_segment()), data.key)
